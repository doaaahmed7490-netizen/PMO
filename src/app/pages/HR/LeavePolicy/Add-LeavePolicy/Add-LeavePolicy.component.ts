...
@Component({
  selector: 'app-Add-LeavePolicy',
  templateUrl: './Add-LeavePolicy.component.html',
  styleUrls: ['./Add-LeavePolicy.component.css']
})
...
VacationTypes=[];
  ...
  ngOnInit() {
     this.options = [
       ...
       this.VacationTypes = [
        { id: 1, label: 'الإجازة السنوية' },
        { id: 2, label: 'الإجازة المرضية' },
        { id: 3, label: 'إجازة الوضع' }
      ];
    this.initForm();
    //this.getDistricts();
  }
  ...
  initForm() {
    this.districtForm = this.formBuilder.group({
      VacationType: ["", [Validators.required, Validators.minLength(3)]],
      Policy:[""],
      AccrualMethod:['No'],
      AllowCarryForward:['No'],
RequiresManagerApproval:['Yes'],
RequiresHRManagerApproval:['Yes'],
  ...
  */
   ...
   onSubmit()
 {
   const returnUrl = this.route.snapshot.queryParams['returnUrl'] || '/LeavePolicy';
