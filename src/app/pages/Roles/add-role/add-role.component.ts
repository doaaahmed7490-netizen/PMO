...
//  nationalId: number;
 ...
 canDelete:boolean;
  ...
  search(page?: PageEvent) {
    ...
    this.role.ListAllPrivlages(this.searchModel).subscribe((res) => {
      this.privilages = res.entity.entities;
...
model.privilage=this.privilageRole;
    /*this.role.addRole(model).subscribe((res) => {
      if (res.status == 200) {
        this.toastrService.Create(EntityNames.Role);
...
else
{
    ...
    this.role.addRole(model).subscribe( {
      next: (res) => {
